            ...
            <?php if (count($products) > 0): ?>
                <?php foreach ($products as $product): ?>
                    <div class="product-item">
                        ...
                        <a href="?delete_id=<?= $product['id']; ?>&type=<?= $product['type']; ?>" onclick="return confirm('Voulez-vous vraiment supprimer ce produit ?');">
                            <button type="button">Supprimer</button>
                        </a>
                        <button type="button" onclick="openEditProductForm('<?= $product['id']; ?>', '<?= $product['type']; ?>', '<?= htmlspecialchars($product['name']); ?>', '<?= htmlspecialchars($product['description']); ?>', '<?= $product['prix']; ?>', '<?= htmlspecialchars($product['picture']); ?>')">Modifier</button>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
