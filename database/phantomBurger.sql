...
--
-- Doublure de structure pour la vue `transactions_utilisateur`
-- (Voir ci-dessous la vue réelle)
--
DROP VIEW IF EXISTS `transactions_utilisateur`;
CREATE TABLE IF NOT EXISTS `transactions_utilisateur` (
);
...
--
-- Structure de la vue `transactions_utilisateur`
--
DROP TABLE IF EXISTS `transactions_utilisateur`;

DROP VIEW IF EXISTS `transactions_utilisateur`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `transactions_utilisateur`  AS SELECT `u`.`utilisateur_id_` AS `utilisateur_id_`, `u`.`nom` AS `nom`, `u`.`prenom` AS `prenom`, `u`.`email` AS `email`, `u`.`date_inscription` AS `date_inscription`, `u`.`role_id` AS `role_id`, `c`.`date_commande` AS `date_commande`, `cd`.`commande_id` AS `commande_id`, `cd`.`menu_id` AS `menu_id`, `cd`.`boisson_id` AS `boisson_id`, `cd`.`burger_id` AS `burger_id`, `cd`.`quantite` AS `quantite`, `cd`.`prix_unitaire` AS `prix_unitaire`, `cd`.`timestamp` AS `transaction_timestamp` FROM ((`utilisateurs` `u` join `commandes_details` `cd` on((`u`.`utilisateur_id_` = `cd`.`utilisateur_id_`))) join `commandes` `c` on((`cd`.`commande_id` = `c`.`commande_id`)))  ;
COMMIT;
